...
Answer:
  ...
  def gamma_and_epsilon(data) do
      ...
      {gamma, ""} = Enum.map(StateStoreD3.value, fn count -> if count > threshhold, do: 1, else: 0 end)
      |> Enum.join
      |> Integer.parse(2)
      
      {epsilon, ""} = Enum.map(StateStoreD3.value, fn count -> if count < threshhold, do: 1, else: 0 end)
      |> Enum.join
      |> Integer.parse(2)
  ...
  def d3_2([head|_tail]=data) do
      ...
      {o2_rating, ""} = filter_data(data, 0, :o2)
            |> IO.inspect
            |> Enum.join
            |> IO.inspect
            |> Integer.parse(2)
            |> IO.inspect
      {co2_rating, ""} = filter_data(data, 0, :co2)
            |> IO.inspect
            |> Enum.join
            |> IO.inspect
            |> Integer.parse(2)
  ...
  def filter_data(data, binary_indx, type = :o2) do
      ...
      if column >= length(data)/2 do
        new_data = Enum.filter(data, fn row -> Enum.at(row, binary_indx) == "1" end)
        |> IO.inspect(label: "1 data ")
        if length(new_data) >= 1, do: filter_data(new_data, binary_indx+1, type), else: data
      else
        new_data = Enum.filter(data, fn row -> Enum.at(row, binary_indx) == "0" end)
    ...
    def filter_data(data, binary_indx, type = :co2) do
      ...
      if column >= length(data)/2 do
        new_data = Enum.reject(data, fn row -> Enum.at(row, binary_indx) == "1" end)
        |> IO.inspect(label: "1 data ")
        if length(new_data) >= 1, do: filter_data(new_data, binary_indx+1, type), else: data
      else
        new_data = Enum.reject(data, fn row -> Enum.at(row, binary_indx) == "0" end)
        |> IO.inspect(label: "0 data ")
        if length(new_data) >= 1, do: filter_data(new_data, binary_indx+1, type), else: data
      end
  end
